...
class ProductListView(generic.ListView):
    ...
    def _get_sorted_list(
        self, queryset: QuerySet[models.Product], sort_by: str
        ...
        if sort_by == "price":
            return sorted(queryset, key=lambda item: item.discount["price"])
        elif sort_by == "-price":
            return sorted(
                queryset, key=lambda item: item.discount["price"], reverse=True
            )
        elif sort_by == "popularity":
            return sorted(
                queryset.annotate(
                    popularity=Coalesce(Sum("stock__order_entity_stock__count"), 0)
                ),
            ...
            return sorted(
                queryset.annotate(reviews=Coalesce(Count("user_product_view"), 0)),
    ...
    def _get_attributes(self) -> List[AttributeDict]:
        ...
        """
        attr_values: list = list(
            models.AttributeValue.objects.only(
                "attribute__id", "attribute__title", "attribute__type", "value"
            )
            .filter(attribute__category__id=self.query_params["category"])
            .values("attribute__id", "attribute__title", "attribute__type", "value")
        )
        attr_values = sorted(attr_values, key=lambda item: item["attribute__id"])
        result: list = []
        current_id: int = 0
        for item in attr_values:
            ...
            if idx == current_id:
                ...
                result.append(
                    {"id": idx, "title": title, "type": type, "values": set((value,))}
                )
                current_id = idx
    ...
    def _get_categories(self) -> List[Optional[int]]:
        ...
        result += [
            item[0]
            for item in models.Category.objects.only("id")
    ...
    def _get_prices_shops(
        self, products: QuerySet[models.Product]
    ) -> Dict[int, Decimal]:
        ...
        """
        self.context["shops"] = list(
            Shop.objects.only("id", "name")
            .filter(Q(stock__count__gt=0) & Q(stock__product__in=products))
            .distinct()
            .order_by("name")
        )
        prices: Dict[int, Decimal] = {
            item.id: item.discount["price"] for item in products
        }
    ...
    def _get_base_filters(self, prices: Dict[int, Decimal]) -> Q:
        ...
        if "price" in self.query_params:
            ...
            if (
                min_price != self.context["min_price"]
                or max_price != self.context["max_price"]
            ):
        ...
        if "shop" in self.query_params:
            self.query_params["shop"] = list(map(int, self.query_params["shop"]))
        ...
        if "tag" in self.query_params:
            self.query_params["tag"] = list(map(int, self.query_params["tag"]))
        ...
        for attr in self.query_params.keys():
            if attr.startswith("attr_"):
                ...
                if type_ == "t":
                    ...
                    result.append(
                        Q(attribute__id=int(id_), value=self.query_params[attr])
                    )
                elif type_ == "c":
                    result.append(Q(attribute__id=int(id_), value="Yes"))
        return result
    ...
    def get_queryset(self):
        ...
        product_list_cache = cache.get(f"product_list_set_{cache_suffix}", default=None)
        context_cache = cache.get(f"product_list_context_{cache_suffix}", default=None)
        ...
        if "category" in self.query_params:
            for filter in self._get_attr_filters():
                result = result.filter(
                    product_item__in=models.AttributeValue.objects.filter(filter)
                )
        result = self._get_sorted_list(result, self.sort_by)
        # Формируем кэш данного набора параметров
        product_list_cache_time_setting: Settings = Settings.objects.filter(
            name="product_list_cache_time"
        ).first()
        ...
        cache.set(f"product_list_set_{cache_suffix}", result, product_list_cache_time)
        cache.set(
    ...
    def post(self, request: HttpRequest, *args, **kwargs):
        ...
        if "query" in self.query_params:
            return redirect(url + "?query=%s" % self.query_params["query"])
        ...
        return redirect(url + "?%s" % urlencode(self.query_params, True))
...
class ProductDetailView(FormMixin, DetailView):
    ...
    def get_queryset(self):
        ...
        if product_id is not None:
            cache_key = PRODUCT_DETAIL_QUERYSET_CACHE_KEY.format(product_id=product_id)
            product_detail_cache = cache.get(cache_key, default=None)
        if product_detail_cache is None:
            queryset = super(ProductDetailView, self).get_queryset()
            product_detail_cache_time_setting: Settings = Settings.objects.filter(
                name="product_detail_cache_time"
            ).first()
            ...
            cache.set(cache_key, queryset, product_detail_cache_time)
        else:
    ...
    def get_context_data(self, **kwargs):
        ...
        cache_key = PRODUCT_DETAIL_CONTEXT_CACHE_KEY.format(product_id=product_id)
        # Если есть кэш данного набора, то возвращаем его
        product_detail_cache = cache.get(cache_key, default=None)
        ...
        if product_detail_cache is None:
            cached_context = dict()
            cached_context["images"] = models.ProductImage.get_product_pics(
                product_on_page
            )
            cached_context[
                "attributes"
            ] = models.AttributeValue.get_all_attributes_of_product(product_on_page)
            cached_context["comments"] = models.ProductReview.get_comments(
                product_on_page
            )
            cached_context["stocks"] = models.Stock.get_products_in_stock(
                product_on_page
            )
            ...
            cache.set(cache_key, cached_context, product_detail_cache_time)
        else:
            cached_context = product_detail_cache

        context.update(**cached_context)

        get_product_detail_view.send(
            sender=self.__class__, user=self.request.user, product=product_on_page.id
        )
