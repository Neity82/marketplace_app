...
class ProductListTest(TestCase):
    ...
    def _get_random_product() -> product_models.Product:
        return product_models.Product.objects.all().order_by("?").first()

    @staticmethod
    def _get_random_category() -> product_models.Category:
        return product_models.Category.objects.all().order_by("?").first()
    ...
    # @pytest.mark.django_db
    def _test_price_filter(
        self, data: dict, price_first: int, price_second: int
    ) -> None:
    ...
    def _test_shop_filter(self, data: dict, shops_filter: list) -> None:
        ...
        for product in products:
            product_shops = [
                item[0]
                for item in product_models.Stock.objects.only("shop__id")
                .filter(product=product)
                .values_list("shop__id")
            ]
            self.assertTrue(
                any([(product_shop in shops_filter) for product_shop in product_shops])
            )
    ...
    def test_sorting(self, ext_data=dict()) -> None:
        ...
        sortings: typing.List[dict] = [
            {"sorting": "", "property_name": "discount", "dict_key": "price"},
            {"sorting": "invalid", "property_name": "discount", "dict_key": "price"},
            {"sorting": "price", "property_name": "discount", "dict_key": "price"},
            {"sorting": "-price", "property_name": "discount", "dict_key": "price"},
            {"sorting": "popularity", "property_name": "popularity", "dict_key": None},
            {"sorting": "-popularity", "property_name": "popularity", "dict_key": None},
            {"sorting": "review", "property_name": "reviews", "dict_key": None},
            {"sorting": "-review", "property_name": "reviews", "dict_key": None},
            {"sorting": "novelty", "property_name": "created_at", "dict_key": None},
            {"sorting": "-novelty", "property_name": "created_at", "dict_key": None},
        ]
        ...
        for item in sortings:
            ...
            if not products:
                return
            self.assertTrue(
                all([hasattr(product, property_name) for product in products])
            )
    ...
    def test_attribute_filters(self) -> None:
        ...
        for attribute in response.context_data["attributes"]:
            ...
            if attribute["type"] == "C":
                selected_value = "on"
                temp_data.update({"attr_c_%d" % attribute["id"]: selected_value})
            elif attribute["type"] == "S":
                selected_value = choice(list(attribute["values"]))
                temp_data.update({"attr_s_%d" % attribute["id"]: selected_value})
            elif attribute["type"] == "T":
                selected_value = choice(list(attribute["values"])).split(sep=" ")[0]
                temp_data.update({"attr_t_%d" % attribute["id"]: selected_value})
            response = self.client.get(self.base_url, temp_data)
            products: list = response.context_data.get("products", [])
            for product in products:
                attr_value = product_models.AttributeValue.objects.filter(
                    Q(product=product) & Q(attribute__id=attribute["id"])
                )
