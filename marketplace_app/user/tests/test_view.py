...
class BaseViewTests(TestCase):
    ...
    def setUpTestData(cls):
        ...
        category = Category.objects.create(title="category", icon=icon)
        cls.count = 5

        for i in range(1, cls.count + 1):
            Product.objects.create(title=f"Product_{i}", category=category)
        for i in range(1, cls.count + 1):
        ...
        for i in range(4):
            CompareEntity.objects.create(
                compare=compare, product=Product.objects.get(id=i + 1)
            )
...
class UserProfileViewTests(BaseViewTests):
    ...
    def test_correct_template(self):
        ...
        self.assertEqual(
            response.context["form"].initial["full_name"], "Last First Middle"
        )
...
class HistoryOrderTests(BaseViewTests):
    ...
    def test_correct_template(self):
        ...
        self.assertQuerysetEqual(
            ...
            ["<Order: Order №3>", "<Order: Order №2>", "<Order: Order №1>"],
        )
